# importing necessary libraries
import streamlit as st

# Page content
st.markdown(" <center>  <h1> Used Car Price Prediction </h1> </font> </center> </h1> ",
            unsafe_allow_html=True)

st.markdown(''' <center>  <h6>
    This app is created to predict Used Car Price in Egypt </center> </h6> ''', unsafe_allow_html=True)

# the path of a photo provided from the path of this file
st.image('./Machine-Learning/Regression/Used_Cars_Price_Prediction/Sources/Cars_Egypt.jpg')
